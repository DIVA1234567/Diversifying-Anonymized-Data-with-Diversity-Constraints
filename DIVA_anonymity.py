...
def checkDiversityConstraint(constraints, dataset):
     for constraint in constraints:
          checkSingleConstraint(constraint, dataset)

def checkSingleConstraint(constraint, dataset):
     lower, attr, upper = constraint[0], constraint[1], constraint[2]
     for record in dataset:
          attrVal = record[attr]
          if lower <= attrVal <= upper:
               return True
          else:
               return False 
...
def generate_min_preserve_clustering(dataset_name, dataset_info, k_size, threshold):
     ...
     # Calculate the classes distances with other classes based on the constraintss
     ...
     while True: 
          ...
          for class_i in list(classes_distance.keys()): 
               ...
               while True: 
                    ...
                    # Calculate the preserve of all patterns with respect to a clustered class 
                    calculate_patterns_preserve_cluster(clustering_info, cluster, curr_pattern_info, changed_pattern_set, greater_preserve_patterns, qid_info, k_size)
          ...
          for _class in best_sealed_cluster:
               ...
               # Iterate through the constraintss to generate keys to store pattern information
               for item in constraints_INFO:
                    pattern_key = None
                    for i in item:
...
if __name__ == '__main__':
     ...
     if 4 == len(sys.argv):
          ...
          try:
               ...
               hierarchy_constraints = open(hierarchy_file, 'rb')
               hierarchy_cache = pickle.load(hierarchy_constraints)
               print("Hierarchy information found in the pickled file -", hierarchy_file) 
               hierarchy_constraints.close()
          except:
               # Do nothing 
               pass 
          try:
               ...
               preserve_constraints = open(preserve_file, 'rb')
               patterns_preserve_cluster_cache = pickle.load(preserve_constraints)
               print("Pattern preserve information found in the pickled file -", preserve_file) 
               preserve_constraints.close()
          except:
               # Do nothing
               pass 
          ...
          hierarchy_constraints = open(hierarchy_file, 'wb')
          pickle.dump(hierarchy_cache, hierarchy_constraints)
          hierarchy_constraints.close()
          # Cache the calculated preserve pattern information  
          preserve_constraints = open(preserve_file, 'wb')
          pickle.dump(patterns_preserve_cluster_cache, preserve_constraints)
          preserve_constraints.close()
